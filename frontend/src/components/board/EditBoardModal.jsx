...
export default function EditBoardModal({ board, onClose, onUpdate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await onUpdate(board.id, { title: title.trim(), description: description.trim(), color });
      onClose();
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-2.5 rounded-lg text-gray-400 bg-gray-800 hover:bg-gray-700 transition-colors font-medium">
              Cancel
            </button>
            <button
              type="submit"
